...
if check_password():
    ...
    st.markdown("사이드바에서 스탑워치를 제어하고 랩타임을 실시간으로 기록하세요!")
    ...
    # 사이드바에 랩 기록 버튼 배치
    if st.sidebar.button("🏁 랩(Lap) 기록하기", use_container_width=True, type="primary"):
        if st.session_state["is_running"] or total_elapsed > 0:
            lap_duration = current_lap_elapsed
            
            if lap_duration > 0.5: # 오작동 방지
                ...
                st.sidebar.success(f"{next_lap_no}랩 기록 ({lap_duration:.2f}초) 저장!")
                st.rerun()
    ...
    st.sidebar.markdown("---")
    if st.sidebar.button("전체 랩 기록 초기화", use_container_width=True):
        st.session_state["lap_data"] = pd.DataFrame(columns=["세션명", "랩 번호", "랩타임(초)"])
        st.rerun()
    ...
    if not session_df.empty:
        ...
        st.info("👈 왼쪽 사이드바에서 **[측정 시작]**을 누른 뒤, 차량이 들어올 때마다 **[🏁 랩(Lap) 기록하기]** 버튼을 누르세요!")

    # 실시간 타이머 작동 중 자동 새로고침
    if st.session_state["is_running"]:
        time.sleep(0.1)
        st.rerun()
